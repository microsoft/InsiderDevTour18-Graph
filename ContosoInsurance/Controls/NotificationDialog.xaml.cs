...
namespace ContosoInsurance.Controls
{
    public sealed partial class NotificationDialog : UserControl
    {
        ...
        private string GetCard(int index) =>
            File.ReadAllText(Package.Current.InstalledLocation.Path + $"\\AdaptiveCards\\bot{index}.json");
        ...
        private void InitializeCardRenderer()
        {
            // Your code goes here
        }

        private Task RenderCardAsync(string card)
        {
            var taskSource = new TaskCompletionSource<bool>();
